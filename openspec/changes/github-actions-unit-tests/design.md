...
The project currently lacks an automated continuous integration (CI) pipeline to verify code changes. Developers must run tests manually before pushing, which is error-prone and can lead to broken code in the `master` branch.

## Goals / Non-Goals

**Goals:**
- Automate unit test execution on Pull Requests from `feature/*` to `master`.
